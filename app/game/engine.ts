...
export class PenaltyEngine {
  ...
  private renderStaticLayer() {
    ...
    // Placas de publicidade atras do gol
    const boardH = boardBottom - boardTop
    ctx.fillStyle = '#101c30'
    ctx.fillRect(0, boardTop, W, boardH)
    const boardColors = ['#1f6feb', '#e8b13f', '#1f6feb', '#31a24c', '#e8b13f', '#1f6feb']
    const bw = W / boardColors.length
    boardColors.forEach((c, i) => {
      ctx.fillStyle = c
      ctx.globalAlpha = 0.22
      ctx.fillRect(i * bw + 2, boardTop + boardH * 0.15, bw - 4, boardH * 0.7)
      ctx.globalAlpha = 1
      ctx.fillStyle = 'rgba(255,255,255,0.55)'
      ctx.font = `700 ${Math.min(boardH * 0.42, bw * 0.15)}px 'Segoe UI', sans-serif`
      ctx.textAlign = 'center'
      ctx.textBaseline = 'middle'
      ctx.fillText('PREMIADO', i * bw + bw / 2, boardTop + boardH * 0.52)
    })
    ...
    const build = (variant: number) => {
      ...
      for (let r = 0; r < rows; r++) {
        ...
        for (let x = step / 2 + (r % 2) * step * 0.5; x < W; x += step) {
          ...
          ctx.fillStyle = CROWD_COLORS[cIdx]!
          ctx.beginPath()
